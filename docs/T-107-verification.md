...
To recover from corruption, rerun normally for rejection/rebuild, use `--no-cache`, or remove a cache directory when no verification process is using it. Never remove active lock files. For an interrupted run use `verify cleanup`; it refuses an active runner, verifies recorded roots and process identities, preserves evidence and marks a running bundle interrupted. Valid contained runs recover through their recorded job identity, including interruption before a server PID is recorded or after an intermediate parent exits. Missing or ambiguous ownership refuses deletion with instructions. Owned scratch deletion refuses linked roots and ancestors. Within a validated owned root it traverses only ordinary directories and unlinks descendant directory/file reparse links nonrecursively, including dangling junctions left by interrupted tests, without inspecting or changing their targets. It clears only the read-only attribute on regular owned files, including nested Git objects left by cancelled tests. Cache inventory and copying still reject links. Cleanup observes cancellation and is idempotent after successful recovery.
