...
New admission requires an in-progress task, including for Founder. Founder can bypass task ownership, but cannot use a completed task to bypass daily accounting. Recovery and non-executing replay remain available after task transitions.
...
On a busy host, `pwsh ./scripts/check-t121.ps1 -Check test -Serialized` runs the full suite with one test project at a time and `DOTNET_PROCESSOR_COUNT=4` for that invocation. It preserves all assertions and process deadlines, writes separate logs, renews the task heartbeat, and restores the caller's environment. This controls local test parallelism only; it does not change conductor capacity or budgets.
