...
Within one checkpoint request, the hub reuses successful Git object/type checks, regular-file blob resolutions keyed by full commit and canonical path, completed ancestry results (including non-ancestry) keyed by the two full commits, and successful branch-name syntax checks. Branch HEAD resolution remains a fresh read. These caches belong only to that request's `TaskUnitGit` instance; the next checkpoint probes again for missing objects, branches, and evidence. Failed, timed-out, or cancelled probes are not cached. This is reuse of observations about immutable objects within a request, not durable verification, and it changes neither proof requirements nor client deadlines.
...
After the request-local proof reuse change, the owner must rerun the full test suite and both installed fixtures against the final assembled candidate with fresh evidence directories. Retain the first checkpoint fixture's HTTP 499 / 30029ms failure (`artifacts/t101-evidence/hub-2.stdout.txt` and `after-14-checkpoint.json`) as failure evidence; it is not a passing run.
