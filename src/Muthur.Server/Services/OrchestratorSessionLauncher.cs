...
public sealed class OrchestratorSessionLauncher(
    ...
    ILogger<OrchestratorSessionLauncher> logger, TimeProvider? clock = null, ITaskLander? lander = null, KnowledgeService? knowledge = null) : IOrchestratorSessionLauncher
{
    ...
    internal static string Prompt(OrchestratorAssignment assignment, HarnessCandidate candidate, string? workKindSuggested = null) =>
        ...
        - Clean up your scratch processes before submitting `muthur task implemented`. Then wait for the verdict in this same session: `muthur msg inbox --wait 90` (costs nothing while it waits). On a failure, fix, re-review, mark implemented again, and wait again — do not exit for a fix you can make right here. Once the verdict passes, or the wait comes back empty after one recheck of `muthur task show`, report the branch/head and completed checks and EXIT; the conductor lands approved work after you exit.
