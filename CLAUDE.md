...
- Tests: `tests/Muthur.Core.Tests` for pure rules; `tests/Muthur.Cli.Tests` for CLI command wiring and query-string construction in the Native AOT pass-through HTTP client (before requests reach the hub); `tests/Muthur.Launch.Tests` for harness adapters; `tests/Muthur.Server.Tests` drive the real HTTP API through `HubFactory` (isolated temp data dir, fake clock, fake PR opener and ingest source, real temp git repos via `TestRepo`). New behavior comes with tests.
- Tests never sleep to synchronize: real time is only ever a hang-detector budget (`Eventually`), and anything a test waits for is a condition it can observe or a step of the fake clock.
